...
class LossFn:
    ...
    def Div_loss_2p(self, input):
        b, s = input[0].size()
        diff0 = input[1] - self.margin
        diff1 = input[0] - self.margin

        div_loss = (torch.sum(input[0] * diff0) + torch.sum(input[1] * diff1)) / b
    ...
    def Dis_loss_2p(self, input):
        ...
        dis_loss = (torch.sum(diff0) + torch.sum(diff1)) / b
    ...
    def group_loss_2p(self, input):
        dis_loss = self.Dis_loss_2p(input)
        div_loss = self.lam * self.Div_loss_2p(input)
        total_loss = dis_loss + div_loss
        return total_loss, dis_loss, div_loss
